...
label_style = {'fontWeight': 'bold', 'fontSize': '13px', 'marginBottom': '4px', 'display': 'block'}
...
def make_sparkline(y_vals, seasons_list, color, y_title):
    ...
    fig.add_trace(go.Scatter(
        x=seasons_list, y=y_vals, mode='lines+markers',
        line=dict(color=color, width=2), marker=dict(size=6),
        hovertemplate='%{x}: %{y:.1f}<extra></extra>',
    ))
    fig.update_layout(
        height=95, margin=dict(l=35, r=10, t=4, b=22),
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Button('← Refine Search', id='back-button', n_clicks=0,
                    style={'display': 'none', 'padding': '8px 16px', 'backgroundColor': 'white',
                           'color': BLUE, 'border': f'2px solid {BLUE}', 'borderRadius': '4px',
                           'cursor': 'pointer', 'fontSize': '13px', 'fontWeight': 'bold'}),
    ], style={'padding': '14px 24px', 'borderBottom': f'3px solid {BLUE}',
    ...
    html.Div([
        ...
        # Single filter panel — always in DOM
        html.Div(id='filter-panel', children=[
            ...
            html.Div(id='cap-context', style={
                'backgroundColor': '#e8eef7', 'border': f'1px solid {BLUE}',
            ...
            html.Div([
                ...
                dcc.Checklist(id='position-filter',
            ...
            html.Div([
                ...
                *[html.Div([
                    ...
                    dcc.Slider(id=f'weight-{dim.lower()}', min=0, max=10, step=1, value=5,
                               marks={0: '0', 5: '5', 10: '10'},
                               tooltip={'placement': 'bottom', 'always_visible': False}),
            ...
            'width': '270px', 'minWidth': '270px', 'padding': '18px',
            'backgroundColor': '#f8f9fa', 'borderRight': '2px solid #ddd',
            'overflowY': 'auto', 'height': 'calc(100vh - 70px)', 'boxSizing': 'border-box',
            'transition': 'width 0.25s ease, min-width 0.25s ease, padding 0.25s ease',
        }),

        # Right side — welcome AND results both always in DOM, toggled via display
        html.Div([

            # Welcome panel (Stage 1)
            html.Div([
                html.Div([
                    html.H2("Welcome to NBA Player Targeting Engine",
                            style={'color': BLUE, 'marginBottom': '12px'}),
                    html.P("This tool helps NBA front offices identify acquisition targets "
                    ...
                    html.Div([
                        html.Div([html.Span("①", style={'fontSize': '22px', 'color': BLUE, 'marginRight': '12px'}),
                                  html.Span("Set your Team Context and cap situation", style={'fontSize': '14px'})],
                                 style={'display': 'flex', 'alignItems': 'center', 'marginBottom': '16px'}),
                        html.Div([html.Span("②", style={'fontSize': '22px', 'color': BLUE, 'marginRight': '12px'}),
                                  html.Span("Filter by position, age, salary, availability, and contract type", style={'fontSize': '14px'})],
                                 style={'display': 'flex', 'alignItems': 'center', 'marginBottom': '16px'}),
                        html.Div([html.Span("③", style={'fontSize': '22px', 'color': BLUE, 'marginRight': '12px'}),
                                  html.Span("Weight your statistical priorities to drive the ranking score", style={'fontSize': '14px'})],
                                 style={'display': 'flex', 'alignItems': 'center', 'marginBottom': '40px'}),
                    ]),
                    html.Button('Find Players →', id='find-button', n_clicks=0,
                                style={'padding': '14px 40px', 'backgroundColor': BLUE,
                ...
                ], style={'maxWidth': '640px', 'margin': 'auto', 'paddingTop': '80px'}),
            ], id='welcome-panel', style={
                'display': 'flex', 'flex': '1', 'alignItems': 'flex-start',
                'justifyContent': 'center', 'padding': '20px', 'overflowY': 'auto',
            }),

            # Results panel (Stage 2) — hidden initially
            html.Div([
                # Toolbar
                html.Div([
                    html.Button('⚙ Filters', id='sidebar-toggle', n_clicks=0,
                                style={'padding': '6px 14px', 'backgroundColor': 'white',
                                       'color': BLUE, 'border': f'1.5px solid {BLUE}',
                ...
                html.Details([
                    html.Summary("How is the Score calculated?",
                                 style={'fontSize': '11px', 'color': BLUE, 'cursor': 'pointer',
                                        'fontWeight': 'bold', 'marginBottom': '4px'}),
                    html.Div([
                        html.P("Each player receives a league percentile rank (0–100) for five stat "
                               "categories. A score of 75 means that player outperforms 75% of the "
                               "league in that dimension.", style={'margin': '4px 0'}),
                    ...
                    ], style={'fontSize': '11px', 'color': '#444', 'backgroundColor': '#f0f4fb',
                              'padding': '8px 10px', 'borderRadius': '4px'}),
                ], style={'marginBottom': '8px'}),

                html.P("Check rows to compare in the charts below. Click any row to load the player detail card.",
                ...
                dash_table.DataTable(
                    ...
                    style_cell_conditional=[
                        ...
                        for c in ['team', 'position', 'age', 'contract_type', 'salary_millions',
                                  'years_remaining', 'guaranteed_millions', 'ppg', 'apg', 'rpg',
                                  'ts_pct_display', 'pie_display', 'availability_pct', 'score']
                    ],
                ...
                html.Div([
                    html.Div([
                        html.H4("Player Comparison (8-axis)", style={'color': BLUE, 'margin': '0 0 2px 0'}),
                        html.P("All axes = league percentile. Check rows to overlay up to 3 players.",
                               style={'fontSize': '11px', 'color': '#888', 'margin': '0 0 2px 0'}),
                        dcc.Graph(id='radar-chart', style={'height': '280px'}, config={'displayModeBar': False}),
                    ], style={'flex': '1', 'minWidth': '0', 'marginRight': '16px'}),
                    html.Div([
                        html.H4("Salary vs. Production", style={'color': BLUE, 'margin': '0 0 2px 0'}),
                        html.P("Gray = all  |  Blue = matches criteria  |  Red ★ = selected. Y: PIE.",
                               style={'fontSize': '11px', 'color': '#888', 'margin': '0 0 2px 0'}),
                        dcc.Graph(id='scatter-chart', style={'height': '280px'}, config={'displayModeBar': False}),
                ...
                html.Div([
                    html.Div([
                        html.H4("Team Fit Gap", style={'color': BLUE, 'margin': '0 0 2px 0'}),
                        html.P(id='fit-gap-subtitle',
                               children="Select a player above to compare against team needs.",
                               style={'fontSize': '11px', 'color': '#888', 'margin': '0 0 2px 0'}),
                        dcc.Graph(id='fit-gap-chart', style={'height': '280px'}, config={'displayModeBar': False}),
                    ], style={'flex': '1', 'minWidth': '0', 'marginRight': '16px'}),
                    html.Div([
                        html.H4("Player Detail", style={'color': BLUE, 'margin': '0 0 2px 0'}),
                        html.P("3-year trends for the first selected player.",
                               style={'fontSize': '11px', 'color': '#888', 'margin': '0 0 2px 0'}),
                        html.Div(id='player-detail-card', style={
                            'height': '280px', 'overflowY': 'auto', 'backgroundColor': '#f8f9fa',
                            'border': '1px solid #ddd', 'borderRadius': '6px', 'padding': '10px',
                        }),
                    ], style={'flex': '1', 'minWidth': '0'}),
                ], style={'display': 'flex'}),

            ], id='results-panel', style={'display': 'none', 'padding': '16px 20px', 'overflowY': 'auto'}),

        ], style={'flex': '1', 'minWidth': '0', 'height': 'calc(100vh - 70px)',
                  'display': 'flex', 'flexDirection': 'column'}),

    ], style={'display': 'flex', 'height': 'calc(100vh - 70px)'}),

    dcc.Store(id='filtered-store'),
    dcc.Store(id='stage-store', data='setup'),
    dcc.Store(id='sidebar-store', data='collapsed'),
...
def switch_stage(find_clicks, back_clicks, toggle_clicks, current_stage, sidebar_state):
    from dash import ctx
    triggered = ctx.triggered_id

    back_visible = {'display': 'block', 'padding': '8px 16px', 'backgroundColor': 'white',
                    'color': BLUE, 'border': f'2px solid {BLUE}', 'borderRadius': '4px',
                    'cursor': 'pointer', 'fontSize': '13px', 'fontWeight': 'bold'}
    back_hidden  = {'display': 'none'}
    ...
    panel_collapsed = {
        'width': '0px', 'minWidth': '0px', 'padding': '0px', 'overflow': 'hidden',
        'backgroundColor': '#f8f9fa', 'borderRight': '0px solid #ddd',
        'height': 'calc(100vh - 70px)', 'boxSizing': 'border-box',
        'transition': 'width 0.25s ease, min-width 0.25s ease, padding 0.25s ease',
    }

    welcome_show = {'display': 'flex', 'flex': '1', 'alignItems': 'flex-start',
                    'justifyContent': 'center', 'padding': '20px', 'overflowY': 'auto'}
    welcome_hide = {'display': 'none'}
    results_show = {'display': 'block', 'padding': '16px 20px', 'overflowY': 'auto', 'flex': '1'}
    results_hide = {'display': 'none'}

    if triggered == 'find-button':
        return welcome_hide, results_show, back_visible, panel_collapsed, 'explore'
    elif triggered == 'back-button':
        return welcome_show, results_hide, back_hidden, panel_expanded, 'setup'
    elif triggered == 'sidebar-toggle' and current_stage == 'explore':
        new_panel = panel_expanded if sidebar_state == 'collapsed' else panel_collapsed
        return welcome_hide, results_show, back_visible, new_panel, 'explore'
    else:
        return welcome_show, results_hide, back_hidden, panel_expanded, 'setup'
...
def update_sidebar_store(_, current):
    return 'expanded' if current == 'collapsed' else 'collapsed'
    ...
    if not selected_rows or not store_data:
        ...
        if player_data:
            for dim_idx, dim in enumerate(dims):
                lines   = [f'<b>{dim}</b>'] + [f'{n}: {rv[dim_idx]:.0f}' for n, rv, _ in player_data]
                tooltip = '<br>'.join(lines)
                avg_r   = sum(rv[dim_idx] for _, rv, _ in player_data) / len(player_data)
                fig.add_trace(go.Scatterpolar(
                    r=[avg_r], theta=[dim], mode='markers',
...
def update_scatter(store_data, selected_rows):
    ...
    fig.add_trace(go.Scatter(
        x=bg['salary_millions'], y=bg['pie_display'], mode='markers',
        marker=dict(color='lightgray', size=5, opacity=0.5), text=bg['player_name'],
        hovertemplate='<b>%{text}</b><br>Salary: $%{x}M<br>PIE: %{y:.1f}<extra></extra>',
        name='All players',
    ))
    if store_data:
        blue_full = df[df['player_name'].isin(matched_names - sel_names)]
        fig.add_trace(go.Scatter(
            x=blue_full['salary_millions'], y=blue_full['pie_display'], mode='markers',
            marker=dict(color=BLUE, size=8, opacity=0.85), text=blue_full['player_name'],
            hovertemplate='<b>%{text}</b><br>Salary: $%{x}M<br>PIE: %{y:.1f}<extra></extra>',
    ...
    fig.update_layout(
        ...
        margin=dict(l=45, r=15, t=15, b=40), paper_bgcolor='rgba(0,0,0,0)',
...
def update_fit_gap(team, selected_rows, store_data):
    ...
    fig.add_trace(go.Bar(
        ...
        marker_color=[RED if v < 40 else BLUE for v in team_vals], opacity=0.8,
        hovertemplate='<b>%{x}</b><br>%{fullData.name} percentile: %{y:.0f}<extra></extra>',
    ))
    fig.add_trace(go.Scatter(
        name='League avg (50th)', x=dims, y=[50] * len(dims),
        mode='lines', line=dict(color='gray', width=1.5, dash='dash'), hoverinfo='skip',
    ))
    ...
    if selected_rows and store_data:
        ...
        for i, row_idx in enumerate(selected_rows[:3]):
            ...
            if player_row.empty:
                continue
            player_row  = player_row.iloc[0]
            player_vals = [round(float(player_row.get(FIT_GAP_DIMS[d][1], 0)) * 100, 1) for d in dims]
            fig.add_trace(go.Bar(
                name=pname, x=dims, y=player_vals,
                marker_color=player_colors[i % len(player_colors)], opacity=0.65,
                hovertemplate='<b>%{x}</b><br>%{fullData.name} percentile: %{y:.0f}<extra></extra>',
            ))
        names    = [records.iloc[r]['player_name'] for r in selected_rows[:3] if r < len(records)]
        subtitle = (f"{team} (blue/red) vs. {', '.join(names)} — "
                    f"league percentile by dimension. Red = team weakness below 40th pct.")
    fig.update_layout(
        barmode='group',
        yaxis=dict(range=[0, 100], title='League Percentile', tickvals=[0, 25, 50, 75, 100]),
        margin=dict(l=45, r=15, t=15, b=40), paper_bgcolor='rgba(0,0,0,0)',
        legend=dict(orientation='h', yanchor='bottom', y=1.02, font=dict(size=10)),
        hovermode='x unified',
    )
...
def update_player_detail(selected_rows, store_data):
    placeholder = html.P(
        ...
        style={'color': '#aaa', 'fontSize': '12px', 'textAlign': 'center', 'marginTop': '30px'}
    )
    if not selected_rows or not store_data:
        return placeholder
    records = pd.DataFrame(store_data)
    ...
    return html.Div([
        html.Div([
            ...
            html.Span(f"  {current_team}  |  {current_pos}", style={'fontSize': '12px', 'color': '#555'}),
        ], style={'marginBottom': '3px'}),
        html.Div(f"${current_sal:.1f}M/yr  |  {yrs} yrs left  |  ${guar:.1f}M guaranteed  |  {contract}",
                 style={'fontSize': '11px', 'color': '#666', 'marginBottom': '8px'}),
        html.Div([
            html.P("PPG", style={'fontSize': '10px', 'color': '#888', 'margin': '0', 'fontWeight': 'bold'}),
            dcc.Graph(figure=make_sparkline(ppg_vals, seasons, BLUE, 'PPG'), config={'displayModeBar': False}),
        ]),
        html.Div([
            html.P("TS%", style={'fontSize': '10px', 'color': '#888', 'margin': '4px 0 0 0', 'fontWeight': 'bold'}),
            dcc.Graph(figure=make_sparkline(ts_vals, seasons, GREEN, 'TS%'), config={'displayModeBar': False}),
        ]),
        html.Div([
            html.P("Availability %", style={'fontSize': '10px', 'color': '#888', 'margin': '4px 0 0 0', 'fontWeight': 'bold'}),
            dcc.Graph(figure=make_sparkline(avail_vals, seasons, RED, 'Avail%'), config={'displayModeBar': False}),
        ]),
    ])
